...
class CdkStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        # Create S3 bucket and provide acccess
        bucket = s3.Bucket(self, f"{prefix}AppS3",
                           bucket_name=Config.S3_BUCKET_NAME,
                           public_read_access=False, enforce_ssl=True)
        ...
        transcribe_policy = iam.Policy(self, f"{prefix}TranscribePolicy",
                                       statements=[
                                           iam.PolicyStatement(
                                               actions=["transcribe:*"],
                                               resources=["*"]
        ...
        task_role.attach_inline_policy(transcribe_policy)

        # Grant access to read the secret in Secrets Manager & S3
        secret.grant_read(task_role)
        bucket.grant_read(task_role)
        bucket.grant_put(task_role)
        bucket.grant_delete(task_role)
